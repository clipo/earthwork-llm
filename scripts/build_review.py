...
def _hillshade(rel, px_m=1.0):
    ...
    alt = math.radians(20)
    cz = math.cos(math.pi/2 - alt)
    sz = math.sin(math.pi/2 - alt)
    gy, gx = np.gradient(rel, px_m)
    slp = np.arctan(np.hypot(gy, gx))
...
def build_aoi(name: str, candidates=None):
    ...
    if not cands:
        ...
        return None
    xs = [c["utm_x"] for c in cands]
    ys = [c["utm_y"] for c in cands]
    xmin = math.floor(min(xs) - PAD_M)
    xmax = math.ceil(max(xs) + PAD_M)
    ymin = math.floor(min(ys) - PAD_M)
    ymax = math.ceil(max(ys) + PAD_M)
    W = int(xmax - xmin)
    H = int(ymax - ymin)
    ...
    lw = _lrm(rel, mask, 40.0)
    ln = _lrm(rel, mask, 10.0)
    hs = _hillshade(rel)
    half = int(THUMB_M / 2)
    for c in cands:
        pc = int(round(c["utm_x"] - xmin))
        pr = int(round(ymax - c["utm_y"]))
        r0, r1 = max(0, pr-half), min(H, pr+half)
        c0, c1 = max(0, pc-half), min(W, pc+half)
        sub_hs = hs[r0:r1, c0:c1]
        sub_lw = lw[r0:r1, c0:c1]
        sub_ln = ln[r0:r1, c0:c1]
        ...
        for a, arr, t, cm in ((ax[0], sub_hs, "hillshade", "gray"),
            ...
            if cm == "RdBu_r":
                v = np.nanpercentile(np.abs(arr), 98) if arr.size else 1
                v = max(v, 0.3)
        ...
        fig.suptitle(f"{c['id']} · {c['source']} · score {c['score']} · LRM {c['lrm_wide_m']} m · {THUMB_M} m",
                     fontsize=9)
        fig.tight_layout()
        fig.savefig(thumbs / f"{c['id']}.png", dpi=80, bbox_inches="tight")
        plt.close(fig)
