...
import numpy as _np  # noqa: E402  (sys.path setup must precede repo-local imports)
...
for i,mid in enumerate(picks):
    ...
    cx=float(gold.loc[mid,"utm15n_easting_m"])
    cy=float(gold.loc[mid,"utm15n_northing_m"])
    dem=fetch(cx,cy)
    dem=np.where(np.isfinite(dem),dem,np.nanmedian(dem[np.isfinite(dem)])).astype("float32")
    geo=classify_geomorphon_simple(dem)
    cands=detect_earthworks(geo,dem,"Find pre-European earthwork mounds")
    gpx=HALF
    gpy=HALF  # mound at tile centre
    best=None
    bd=1e9
    for c in cands:
    ...
    ax.set_title(f"{mid}  {d_m:.0f} m",fontsize=9,color="green")
    ax.set_xticks([])
    ax.set_yticks([])
# histogram + summary from UTM offsets
d=rec["dist_m"]
axh=fig.add_subplot(gs[0,4])
axh.hist(d,bins=np.arange(0,32,3),color="#2e7d32",edgecolor="k")
axh.axvline(d.median(),color="red",ls="--",lw=1.5)
axh.set_title(f"Offset, all {len(rec)} recovered",fontsize=8)
axh.set_xlabel("distance (m)",fontsize=8)
axh.set_ylabel("mounds",fontsize=8)
